      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            style={[styles.chip, { backgroundColor: (item.id === "all" ? !selectedCategory : selectedCategory === item.id) ? colors.primary : colors.card }]}
          >
            <Text style={styles.chipEmoji}>{item.icon}</Text>
            <Text style={[styles.chipText, { color: (item.id === "all" ? !selectedCategory : selectedCategory === item.id) ? "#fff" : colors.foreground }]}>{item.name}</Text>
          </Pressable>
        )}
      />
        ...
        renderItem={({ item }) => (
          <View style={[styles.productCard, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.productActions}>
              <Switch value={item.available} trackColor={{ true: colors.primary, false: colors.border }} onValueChange={() => Alert.alert("", "Disponibilidade disponível na versão completa")} />
              <Pressable onPress={() => Alert.alert("", "Edição disponível na versão completa")} style={styles.editBtn}>
...
const styles = StyleSheet.create({
  ...
  productCard: { flexDirection: "row", borderRadius: 16, overflow: "hidden", alignItems: "center" },
  productImage: { width: 72, height: 72, alignItems: "center", justifyContent: "center" },
  ...
  origPrice: { fontSize: 12, fontFamily: "Inter_400Regular", textDecorationLine: "line-through" },
  price: { fontSize: 15, fontFamily: "Inter_700Bold" },
  productActions: { paddingRight: 12, alignItems: "center", gap: 6 },
  editBtn: { padding: 6 },
});
